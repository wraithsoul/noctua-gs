...
interface = {} do
    ...
    interface.utility = {
        ...
        grenade_release_disablers = interface.header.general:multiselect('grenade release disablers', 'he', 'molotov', 'smoke'),
...
--@region: grenade release
grenade_release = {} do
    grenade_release.ref = pui.reference('misc', 'miscellaneous', 'automatic grenade release')

    grenade_release.get_weapon_type = function(weapon)
        local weapon_info = weapon and csgo_weapons(weapon) or nil
        if weapon_info == nil then
            return nil
        end

        if weapon_info.idx == 44 then
            return 'he'
        end

        if weapon_info.idx == 45 then
            return 'smoke'
        end

        if weapon_info.idx == 48 or weapon_info.idx == 46 then
            return 'molotov'
        end
    ...
    grenade_release.should_disable = function()
        local local_player = entity.get_local_player()
        if not local_player or not entity.is_alive(local_player) then
            return false
        end

        local disablers = interface.utility.grenade_release_disablers:get()
        if not disablers or #disablers == 0 then
            return false
        end

        local weapon = entity.get_player_weapon(local_player)
        local weapon_type = grenade_release.get_weapon_type(weapon)
        if weapon_type == nil then
            return false
        end

        return utils.contains(disablers, weapon_type)
    end

    grenade_release.on_setup_command = function()
        if grenade_release.should_disable() then
            grenade_release.ref:override(false)
        end
    end

    grenade_release.on_paint_ui = function()
        grenade_release.ref:override()
    end

    grenade_release.on_shutdown = function()
        grenade_release.ref:override()
    end

    client.set_event_callback('setup_command', grenade_release.on_setup_command)
    client.set_event_callback('paint_ui', grenade_release.on_paint_ui)
    client.set_event_callback('shutdown', grenade_release.on_shutdown)
end
--@endregion
...
configs = {} do
    ...
    local default_config = "noctua:"
    ...
    local function append_byte(parts, value)
    ...
    local function write_varint(parts, value)
        repeat
            ...
            append_byte(parts, byte)
        until value == 0
    end

    local function write_signed_varint(parts, value)
        if value >= 0 then
            write_varint(parts, value * 2)
            return
        end

        write_varint(parts, (-value * 2) - 1)
    end

    local function write_string_blob(parts, value)
        write_varint(parts, #value)
        if value ~= '' then
            parts[#parts + 1] = value
        end
    end
    ...
    local function write_typed_value(parts, value)
        local value_type = type(value)

        if value == nil then
            append_byte(parts, TYPE_NIL)
            return
        end

        if value_type == 'boolean' then
            append_byte(parts, value and TYPE_TRUE or TYPE_FALSE)
            return
        end

        if value_type == 'number' then
            if value == math.floor(value) then
                append_byte(parts, TYPE_INT)
                write_signed_varint(parts, value)
            else
                append_byte(parts, TYPE_NUMBER)
                write_string_blob(parts, string.format('%.14g', value))
            end
            return
        end

        if value_type == 'string' then
            append_byte(parts, TYPE_STRING)
            write_string_blob(parts, value)
            return
        end
        ...
        if is_array then
            append_byte(parts, TYPE_ARRAY)
            write_varint(parts, max_index)
            for i = 1, max_index do
                write_typed_value(parts, value[i])
            end
            return
        end

        append_byte(parts, TYPE_OBJECT)
        ...
        table.sort(keys)
        write_varint(parts, #keys)

        for _, key in ipairs(keys) do
            write_string_blob(parts, key)
            write_typed_value(parts, value[key])
        end
    end

    local function reader_new(data)
        return {
            data = data,
            pos = 1,
            len = #data
    ...
    local function read_byte(reader)
        if reader.pos > reader.len then
            error('unexpected end of packed config')
        end

        local value = string.byte(reader.data, reader.pos)
        reader.pos = reader.pos + 1
        return value
    end

    local function read_varint(reader)
        local value = 0
        local factor = 1

        while true do
            local byte = read_byte(reader)
    ...
    local function read_signed_varint(reader)
        local value = read_varint(reader)
        if value % 2 == 0 then
            return value / 2
        end
    ...
    local function read_string_blob(reader)
        local length = read_varint(reader)
        if length == 0 then
            return ''
        end

        local start_pos = reader.pos
        local end_pos = start_pos + length - 1
        if end_pos > reader.len then
            error('packed config string exceeds payload size')
        end

        reader.pos = end_pos + 1
        return string.sub(reader.data, start_pos, end_pos)
    end

    local function read_typed_value(reader)
        local tag = read_byte(reader)

        if tag == TYPE_NIL then
            return nil
        end

        if tag == TYPE_FALSE then
            return false
        end

        if tag == TYPE_TRUE then
            return true
        end

        if tag == TYPE_INT then
            return read_signed_varint(reader)
        end

        if tag == TYPE_NUMBER then
            local parsed = tonumber(read_string_blob(reader))
            if parsed == nil then
                error('packed config contains invalid numeric value')
            end
            return parsed
        end

        if tag == TYPE_STRING then
            return read_string_blob(reader)
        end

        if tag == TYPE_ARRAY then
            local count = read_varint(reader)
            local out = {}
            for i = 1, count do
                out[i] = read_typed_value(reader)
            end
            return out
        end

        if tag == TYPE_OBJECT then
            local count = read_varint(reader)
            local out = {}
            for _ = 1, count do
                local key = read_string_blob(reader)
                out[key] = read_typed_value(reader)
            end
            return out
        end

        error('packed config contains unknown tag: ' .. tostring(tag))
    end

    local function split_config_key(path)
        local out = {}
        local start_pos = 1

        while true do
            local dot = string.find(path, '.', start_pos, true)
            if dot then
                out[#out + 1] = string.sub(path, start_pos, dot - 1)
                start_pos = dot + 1
            else
                out[#out + 1] = string.sub(path, start_pos)
                return out
            end
        end
    end

    local function normalize_payload(data, config_name)
        if type(data) ~= 'table' then
            return nil
        end

        local values = type(data.values) == 'table' and data.values or data
        if type(values) ~= 'table' then
            return nil
        end

        local payload = {
            version = tonumber(data.version) or PACK_VERSION,
            values = values,
            widgets = type(data.widgets) == 'table' and data.widgets or {}
        }

        local final_name = config_name or data.config_name
        if type(final_name) == 'string' and final_name ~= '' then
            payload.config_name = final_name
        end
    ...
    local function encode_payload(payload)
        payload = normalize_payload(payload, payload and payload.config_name)
        if not payload then
            error('invalid config payload')
        end

        local keys = {}
        for key in pairs(payload.values) do
            keys[#keys + 1] = key
        end
        ...
        for i, key in ipairs(keys) do
            local path = split_config_key(key)
            local indices = {}

            for j, segment in ipairs(path) do
                local index = segment_index[segment]
                if index == nil then
                    index = #segments + 1
        ...
        local parts = { PACK_MAGIC, string.char(PACK_VERSION) }
        write_typed_value(parts, payload.config_name)

        write_varint(parts, #segments)
        for i = 1, #segments do
            write_string_blob(parts, segments[i])
        end

        write_varint(parts, #key_paths)

        local previous_path = {}
        for i = 1, #key_paths do
            local current_path = key_paths[i]
            local shared = 0
            local limit = math.min(#previous_path, #current_path)

            while shared < limit and previous_path[shared + 1] == current_path[shared + 1] do
                shared = shared + 1
            end

            write_varint(parts, shared)
            write_varint(parts, #current_path - shared)

            for j = shared + 1, #current_path do
                write_varint(parts, current_path[j] - 1)
            end

            write_typed_value(parts, payload.values[keys[i]])
            previous_path = current_path
        end

        write_typed_value(parts, payload.widgets)

        return table.concat(parts)
    end

    local function decode_packed_payload(data)
        local reader = reader_new(data)
        local magic = string.sub(data, 1, #PACK_MAGIC)
        if magic ~= PACK_MAGIC then
            error('packed config magic mismatch')
        end

        reader.pos = #PACK_MAGIC + 1
        local version = read_byte(reader)
        if version ~= PACK_VERSION then
            error('unsupported packed config version: ' .. tostring(version))
        end

        local config_name = read_typed_value(reader)
        if config_name ~= nil and type(config_name) ~= 'string' then
            error('packed config contains invalid config name')
        end

        local segment_count = read_varint(reader)
        local segments = {}
        for i = 1, segment_count do
            segments[i] = read_string_blob(reader)
        end

        local value_count = read_varint(reader)
        local values = {}
        local previous_path = {}

        for _ = 1, value_count do
            local shared = read_varint(reader)
            local suffix = read_varint(reader)
            if shared > #previous_path then
                error('packed config path prefix exceeds previous key length')
            end

            local current_path = {}
            for i = 1, shared do
                current_path[i] = previous_path[i]
            end

            for i = 1, suffix do
                local segment_id = read_varint(reader) + 1
                if segments[segment_id] == nil then
                    error('packed config references unknown segment id')
                end
                current_path[shared + i] = segment_id
            end

            local path_parts = {}
            for i = 1, #current_path do
                path_parts[i] = segments[current_path[i]]
            end

            values[table.concat(path_parts, '.')] = read_typed_value(reader)
            previous_path = current_path
        end

        local widgets_data = read_typed_value(reader)
        if widgets_data == nil then
            widgets_data = {}
        end

        if type(widgets_data) ~= 'table' then
            error('packed config contains invalid widgets payload')
        end

        if reader.pos <= reader.len then
            error('packed config contains trailing bytes')
        end
    ...
    local function decode_payload(data)
        if type(data) ~= 'string' or data == '' then
            return nil, 'empty config payload'
        end

        if string.sub(data, 1, #PACK_MAGIC) == PACK_MAGIC then
            local ok, payload = pcall(decode_packed_payload, data)
            if not ok then
                return nil, payload
            end
    ...
    local function configs_log(fmt, ...)
        argLogWithPrefix('noctua · config', fmt, ...)
    end
    ...
    function configs.load_db()
        ...
        if ok and type(data) == 'table' then
            local raw_list = type(data.list) == 'table' and data.list or {}
            local raw_data = type(data.data) == 'table' and data.data or {}
            local list = {}
            local normalized_data = {}
            local seen = {}

            for _, name in ipairs(raw_list) do
                if type(name) == 'string' and not seen[name] then
                    local payload = normalize_payload(raw_data[name], name)
                    if payload then
                        list[#list + 1] = name
                        normalized_data[name] = payload
                        seen[name] = true
                    end
                end
            end

            for name, payload in pairs(raw_data) do
                if type(name) == 'string' and not seen[name] then
                    local normalized = normalize_payload(payload, name)
                    if normalized then
                        list[#list + 1] = name
    ...
    function configs.collect(config_name)
        local out = { version = PACK_VERSION, values = {}, widgets = {} }
        if config_name then
    ...
    function configs.load_default()
        if not default_config or default_config == '' then
            configs_log('default config is empty')
            return
        end
        ...
        if not decoded or decoded == '' then
            configs_log('failed to decode default base64')
            client.exec("play ui/menu_invalid.wav")
            return
        end
        local data = decode_payload(decoded)
        if not data then
            configs_log('failed to parse default config')
            client.exec("play ui/menu_invalid.wav")
            return
        end
        configs.apply(data)
        configs_log('loaded config %s', 'default')
    ...
    function configs.create(name)
        ...
        if name == '' or name == '<no configs>' or name == '<default>' or not name:match('%S') then
            configs_log('enter a valid config name')
            client.exec("play ui/menu_invalid.wav")
            return
        end
        if state.data[name] ~= nil then
            configs_log('config %s already exists', name)
            client.exec("play ui/menu_invalid.wav")
            return
        end
        ...
        configs_log('created config %s', name)
        client.exec("play ui/beepclear.wav")
    end
    ...
    function configs.export_to_clipboard()
        ...
        if config_name == '+ new' then
            configs_log('select a valid config first')
            client.exec("play ui/menu_invalid.wav")
            return
        end
        local payload = configs.collect(config_name)
        local enc = b64_encode(encode_payload(payload))
        clipboard.set('noctua:' .. enc)
        configs_log('exported config %s to clipboard', config_name or 'unknown')
        client.exec("play ui/beepclear.wav")
    end
    ...
    function configs.import_from_clipboard()
        ...
        clip = clip:gsub('^%s+', ''):gsub('%s+$', '')
        if clip == '' then
            configs_log('clipboard is empty')
            client.exec("play ui/menu_invalid.wav")
            return
        end
        local decoded = b64_decode(clip)
        if not decoded or decoded == '' then
            configs_log('failed to decode base64')
            client.exec("play ui/menu_invalid.wav")
            return
        end
        local data = decode_payload(decoded)
        if not data then
            configs_log('failed to parse config, invalid structure')
            client.exec("play ui/menu_invalid.wav")
            return
        end
        configs.apply(data)
        ...
        if config_name and config_name ~= 'default' and config_name ~= '+ new' then
            ...
            state.data[final_name] = normalize_payload(data, final_name)
            configs.save_db()
            configs.update_list_ui()
        end
        configs_log('imported config %s', data.config_name or 'unnamed')
        client.exec("play ui/beepclear.wav")
    end
    ...
    function configs.save_selected()
        ...
        if not name then
            configs_log('select a config first')
            client.exec("play ui/menu_invalid.wav")
            return
        end
        if name == 'default' or name == '+ new' then
            configs_log('cannot overwrite default config')
            client.exec("play ui/menu_invalid.wav")
            return
        end
        state.data[name] = configs.collect()
        configs.save_db()
        widgets.save_all()
        configs_log('saved config %s', name)
        client.exec("play ui/beepclear.wav")
    end
    ...
    function configs.load_selected()
        ...
        if name == '+ new' then
            configs_log('create a new config first')
            client.exec("play ui/menu_invalid.wav")
            return
        end
        local data = state.data[name]
        data = normalize_payload(data, name)
        if not data then
            configs_log('config %s data is invalid', name)
            client.exec("play ui/menu_invalid.wav")
            return
        end
        ...
        configs_log('loaded config %s', name)
        client.exec("play ui/beepclear.wav")
    end
    ...
    function configs.delete_selected()
        ...
        if name == 'default' or name == '+ new' then
            configs_log('cannot delete default config')
            client.exec("play ui/menu_invalid.wav")
            return
        end
        state.data[name] = nil
        ...
        configs.update_list_ui()
        interface.config.list:set(0)
        configs_log('deleted config %s', name)
        client.exec("play ui/beepclear.wav")
    end
    ...
    function configs.load_startup_config()
        ...
        if state.data[state.load_on_startup] then
            ...
            configs_log('autoloaded config %s', state.load_on_startup)
        else
            configs_log('autoload config %s was not found', state.load_on_startup)
        end
    end
